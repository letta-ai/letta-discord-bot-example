...
const client = new letta_client_1.default({
 ...
 */
async function findBlockByLabel(label) {
    try {
 ...
 */
async function createUserBlock(userId) {
 ...
 */
async function getOrCreateUserBlock(userId) {
    const label = `${USER_BLOCK_LABEL_PREFIX}${userId}`;
 ...
 */
async function attachBlockToAgent(blockId) {
    if (!AGENT_ID)
        return false;
    try {
 ...
 */
async function detachBlockFromAgent(blockId) {
    if (!AGENT_ID)
        return false;
    try {
        // SDK v1.0: first param is blockId, second is { agent_id }
 ...
 */
async function attachUserBlocks(senderId, messageContent) {
 ...
 */
async function detachUserBlocks(blockIds) {
 ...
 */
async function cleanupUserBlocks() {
    ...
    try {
        ...
        const userBlocks = agentBlocks.filter(block => block.label && block.label.startsWith(USER_BLOCK_LABEL_PREFIX));
